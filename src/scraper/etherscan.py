...
class EtherScan(BaseScraper):
   def __init__(self, address):
      ...
      #super().__init__(use_proxies=True)
   ...
   async def tes(self):
      ...
      response = await self.scrape(
         url=self.url,
         timeout=10,
         proccessor=self.process_text
      )
      #print(await self.scrape_info_(response.content, 'span.page-link.text-nowrap'))
